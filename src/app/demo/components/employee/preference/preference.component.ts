...
export class PreferenceComponent implements OnInit {
    ...
    in(array: Employee[] | undefined, item: Employee) {
        if (array != undefined) {
            return array?.filter((val, _index) => val == item._id).length > 0;
        }
        return false;
    }
    getAppoints() {
        this.appointmentService.getAppointment('', { status: 0 }, (res) => {
            this.appointments =this.changeResultat(res);
    ...
    // changement resulattselon les preference
    changeResultat(res: Appointment[]) {
        res = res.filter(
            (val, _index) =>
                this.in(val.customer?.preference?.employee, this.getEmp()) ==
                true
        );
        return res;
    }
    onStatusChange(event: any) {
        ...
        this.appointmentService.getAppointment(
            ...
            (res) => {
                if (event.value != -1) {
                    this.appointments = this.findByStatus(this.changeResultat(res), event.value);
                } else {
                    this.appointments = this.changeResultat(res);
                }
            }
